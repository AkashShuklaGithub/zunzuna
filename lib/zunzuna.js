...
function create(id, source, destination, startTravelAt, email) {
    ...
    return getTravelTime(source, destination).then(function(travelTime) {
        ...
        return addEvent(id, source, destination, startTravelAt, email, notificationTime, travelTime, updateTravelTime, updateWaitingTimeAt)
    });
};
...
function zunzuna() {
    ...
    this.createEvent = (event) => {
        ...
        event = create(event.id, event.source, event.destination, event.time, event.emailid);
        var self = this;
        // set the interval
